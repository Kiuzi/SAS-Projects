...
*****************************************************************************************************************;
*****************************************************************************************************************;
* per request that came from Staphanie on Dec 7;
			
					proc sql; create table temp as select * from Hypwork.list5 where SWC_PAT_ID in  (select SWC_PAT_ID from hypwork.dat7 where group2014=1);quit;
					proc sql; select count (distinct SWC_PAT_ID) from temp2;run;
					proc sort  data=temp2; by descending PostIDX_LDL labdate ;run;
					
					proc sql; create table temp2 as select * from Hypwork.Labs2LDL_NHDL3 where SWC_PAT_ID in  (select SWC_PAT_ID from hypwork.dat7 where group2014=1);quit;
